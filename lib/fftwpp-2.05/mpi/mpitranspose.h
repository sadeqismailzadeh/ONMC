...
public:
  ...
  void setup(T *data, MPI_Comm Communicator) {
    if(N < n) ArrayFFT::ArrayFFTExit("N must be >= n");
    if(M < m) ArrayFFT::ArrayFFTExit("M must be >= m");
  ...
  void init(T *data) {
    ...
    if(compact) work=data;
      ...
      if(work == NULL) {
        ...
        ArrayFFT::newAlign(work,allocated,sizeof(T));
      }
    }
  ...
  void deallocate() {
    ...
    if(compact) work=NULL;
      ...
      ArrayFFT::deleteAlign(work,allocated);
      work=NULL;
      allocated=0;
    }
