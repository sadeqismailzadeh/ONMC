...
typedef ArrayFFT1<double>::opt Vector;
...
// ArrayFFTs with offsets:
        
  const int offx=-1;
  const int offy=-1;
        
  ArrayFFT1<double> B(n,offx); // B(offx)...B(n-1+offx)
  ArrayFFT2<double> C(n,m,offx,offy);
  ArrayFFT1<double> D(5);      // Functionally equivalent to array1<double> D(n);
