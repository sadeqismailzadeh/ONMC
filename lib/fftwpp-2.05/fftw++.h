...
#pragma omp parallel for num_threads(threads)
  ...
  threaddata Setup(Complex *in, Complex *out=NULL) {
    ...
    if(alloc) ArrayFFT::deleteAlign(in,(doubles+1)/2);
    return data;
  }
...
#ifdef __ArrayFFT_h__
  fft1d(int sign, const ArrayFFT::array1<Complex>& in,
        const ArrayFFT::array1<Complex>& out=ArrayFFT::NULL1,
        unsigned int threads=maxthreads) 
...
#ifdef __ArrayFFT_h__
  fft2d(int sign, const ArrayFFT::array2<Complex>& in,
        const ArrayFFT::array2<Complex>& out=ArrayFFT::NULL2, 
        unsigned int threads=maxthreads) 
...
#ifdef __ArrayFFT_h__
  fft3d(int sign, const ArrayFFT::array3<Complex>& in,
        const ArrayFFT::array3<Complex>& out=ArrayFFT::NULL3,
        unsigned int threads=maxthreads) 
