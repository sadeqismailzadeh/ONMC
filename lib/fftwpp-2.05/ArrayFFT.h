/* ArrayFFT.h:  A high-performance multi-dimensional C++ array class
...
#ifndef __ArrayFFT_h__
#define __ArrayFFT_h__ 1

#define __ARRAY_H_VERSION__ 1.52

// Defining NDEBUG improves optimization but disables argument checking.
// Defining __NOARRAY2OPT inhibits special optimization of ArrayFFT2[].
...
inline void ArrayFFTExit(const char *x);
  
#ifndef __ExternalArrayFFTExit
inline void ArrayFFTExit(const char *x)
{
...
#endif  
  if(rc == EINVAL) ArrayFFT::ArrayFFTExit(invalid);
  if(rc == ENOMEM) ArrayFFT::ArrayFFTExit(nomem);
  v=(T *) mem;
...
public:
  ...
  void CheckActivate(int dim, size_t align=0) {
    if (test(allocated)) {
      std::ostringstream buf;
      buf << "Reallocation of ArrayFFT" << dim
  ...
  T Min() {
    if(size == 0)
      ArrayFFTExit("Cannot take minimum of empty array"); 
    T min=v[0];
  ...
  T Max() {
    if(size == 0)
      ArrayFFTExit("Cannot take maximum of empty array"); 
    T max=v[0];
...
public:
  ...
  void Dimension(const array1<T> &A) {ArrayFFTExit("Operation not implemented");} 
...
template<class T>
class ArrayFFT1 : public array1<T> {
protected:
...
public:
  ...
  void Dimension(const ArrayFFT1<T>& A) {
  ...
  ArrayFFT1() : ox(0) {}
  ArrayFFT1(unsigned int nx0, int ox0=0, size_t align=0) {
    Allocate(nx0,ox0,align);
  }
  ArrayFFT1(unsigned int nx0, T *v0, int ox0=0) {
    Dimension(nx0,v0,ox0);
  }
  ArrayFFT1(T *v0, int ox0=0) {
  ...
  ArrayFFT1<T> operator + (int i) const {return ArrayFFT1<T>(this->size-i,this->v+i,ox);}
  void Set(T *a) {this->v=a; Offsets(); this->clear(this->allocated);}
        
  ArrayFFT1<T>& operator = (T a) {this->Load(a); return *this;}
  ArrayFFT1<T>& operator = (const T *a) {this->Load(a); return *this;}
  ArrayFFT1<T>& operator = (const ArrayFFT1<T>& A) {
  ...
  ArrayFFT1<T>& operator = (const array1<T>& A) {
    __checkEqual(this->size,A.Size(),1,1);
...
template<class T>
class ArrayFFT2 : public array2<T> {
protected:
  T *voff,*vtemp;
  int ox,oy;
public:
  ...
  ArrayFFT2() : ox(0), oy(0) {}
  ArrayFFT2(unsigned int nx0, unsigned int ny0, int ox0=0, int oy0=0,
         size_t align=0) {
    Allocate(nx0,ny0,ox0,oy0,align);
  }
  ArrayFFT2(unsigned int nx0, unsigned int ny0, T *v0, int ox0=0, int oy0=0) {
...
#else
  ArrayFFT1<T> operator [] (int ix) const {
    __check(ix,this->nx,ox,2,1);
    return ArrayFFT1<T>(this->ny,vtemp+ix*(int) this->ny,oy);
  }
#endif
  ...
  ArrayFFT2<T>& operator = (T a) {this->Load(a); return *this;}
  ArrayFFT2<T>& operator = (T *a) {this->Load(a); return *this;}
  ArrayFFT2<T>& operator = (const ArrayFFT2<T>& A) {
  ...
  ArrayFFT2<T>& operator = (const array2<T>& A) {
    __checkEqual(this->nx,A.Nx(),2,1);
    __checkEqual(this->ny,A.Ny(),2,2);
...
template<class T>
class ArrayFFT3 : public array3<T> {
protected:
  T *voff,*vtemp;
  int ox,oy,oz;
public:
  ...
  ArrayFFT3() : ox(0), oy(0), oz(0) {}
  ArrayFFT3(unsigned int nx0, unsigned int ny0, unsigned int nz0,
         int ox0=0, int oy0=0, int oz0=0, size_t align=0) {
    Allocate(nx0,ny0,nz0,ox0,oy0,oz0,align);
  }
  ArrayFFT3(unsigned int nx0, unsigned int ny0, unsigned int nz0, T *v0,
         int ox0=0, int oy0=0, int oz0=0) {
    Dimension(nx0,ny0,nz0,v0,ox0,oy0,oz0);
  }
        
  ArrayFFT2<T> operator [] (int ix) const {
    __check(ix,this->nx,ox,3,1);
    return ArrayFFT2<T>(this->ny,this->nz,vtemp+ix*(int) this->nyz,oy,oz);
  }
  T& operator () (int ix, int iy, int iz) const {
    __check(ix,this->nx,ox,3,1);
  ...
  ArrayFFT3<T>& operator = (T a) {this->Load(a); return *this;}
  ArrayFFT3<T>& operator = (T *a) {this->Load(a); return *this;}
  ArrayFFT3<T>& operator = (const ArrayFFT3<T>& A) {
  ...
  ArrayFFT3<T>& operator = (const array3<T>& A) {
    __checkEqual(this->nx,A.Nx(),3,1);
    __checkEqual(this->ny,A.Ny(),3,2);
    __checkEqual(this->nz,A.Nz(),3,3);
...
template<class T>
class ArrayFFT4 : public array4<T> {
protected:
  T *voff,*vtemp;
  int ox,oy,oz,ow;
public:
  ...
  ArrayFFT4() : ox(0), oy(0), oz(0), ow(0) {}
  ArrayFFT4(unsigned int nx0, unsigned int ny0, unsigned int nz0,
         unsigned int nw0,
         int ox0=0, int oy0=0, int oz0=0, int ow0=0, size_t align=0) {
  ...
  ArrayFFT3<T> operator [] (int ix) const {
    __check(ix,this->nx,ox,3,1);
    return ArrayFFT3<T>(this->ny,this->nz,this->nw,vtemp+ix*(int) this->nyzw,
                     oy,oz,ow);
  }
  T& operator () (int ix, int iy, int iz, int iw) const {
  ...
  ArrayFFT4<T>& operator = (T a) {this->Load(a); return *this;}
  ArrayFFT4<T>& operator = (T *a) {this->Load(a); return *this;}
  
  ArrayFFT4<T>& operator = (const ArrayFFT4<T>& A) {
  ...
  ArrayFFT4<T>& operator = (const array4<T>& A) {
    __checkEqual(this->nx,A.Nx(),4,1);
    __checkEqual(this->ny,A.Ny(),4,2);
    __checkEqual(this->nz,A.Nz(),4,3);
    __checkEqual(this->nw,A.N4(),4,4);
...
template<class T>
class ArrayFFT5 : public array5<T> {
protected:
  T *voff,*vtemp;
  int ox,oy,oz,ow,ov;
public:
  ...
  ArrayFFT5() : ox(0), oy(0), oz(0), ow(0), ov(0) {}
  ArrayFFT5(unsigned int nx0, unsigned int ny0, unsigned int nz0,
  ...
  ArrayFFT4<T> operator [] (int ix) const {
    __check(ix,this->nx,ox,4,1);
    return ArrayFFT4<T>(this->ny,this->nz,this->nw,this->nv,
  ...
  ArrayFFT5<T>& operator = (T a) {this->Load(a); return *this;}
  ArrayFFT5<T>& operator = (T *a) {this->Load(a); return *this;}
  
  ArrayFFT5<T>& operator = (const ArrayFFT5<T>& A) {
  ...
  ArrayFFT5<T>& operator = (const array5<T>& A) {
    __checkEqual(this->nx,A.Nx(),5,1);
    __checkEqual(this->ny,A.Ny(),5,2);
    __checkEqual(this->nz,A.Nz(),5,3);
    __checkEqual(this->nw,A.N4(),5,4);
...
template<class T>
inline void Set(ArrayFFT1<T>& A, T *v)
{
  A.Set(v);
}

template<class T>
inline void Set(ArrayFFT1<T>& A, const array1<T>& B)
{
  A.Set(B());
}
...
template<class T>
inline void Dimension(ArrayFFT1<T>& A, unsigned int n, T *v)
{
  A.Dimension(n,v,0);
}
...
template<class T>
inline void Dimension(ArrayFFT1<T>& A, const ArrayFFT1<T>& B)
{
  A.Dimension(B);
}

template<class T>
inline void Dimension(ArrayFFT1<T>& A, const array1<T>& B)
{
  A.Dimension(B);
}
...
template<class T>
inline void Dimension(ArrayFFT1<T>& A, unsigned int n, const array1<T>& B, int o)
{
  A.Dimension(n,B,o);
}

template<class T>
inline void Dimension(ArrayFFT1<T>& A, unsigned int n, T *v, int o)
{
  A.Dimension(n,v,o);
}
...
template<class T>
inline void Allocate(ArrayFFT1<T>& A, unsigned int n, size_t align=0)
{  
  A.Allocate(n,align);
}
...
template<class T>
inline void Allocate(ArrayFFT1<T>& A, unsigned int n, int o, size_t align=0)
{  
...
template<class T>
inline void Deallocate(ArrayFFT1<T>& A)
{  
  A.Deallocate();
}
...
template<class T>
inline void Deallocate(ArrayFFT1<T>& A, int)
{  
  A.Deallocate();
}
...
template<class T>
inline void Reallocate(ArrayFFT1<T>& A, unsigned int n)
{  
  A.Reallocate(n);
}
...
template<class T>
inline void Reallocate(ArrayFFT1<T>& A, unsigned int n, int o, size_t align=0)
{  
